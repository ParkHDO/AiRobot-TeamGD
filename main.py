...
object_messages = load_object_messages('object_messages2.json')
...
class FaceManager:
    ...
    def save_faces(self, faces, folder_name):
        ...
        print(f"New faces saved and embeddings updated in {folder_path}.")  # 얼굴 저장 완료 메시지 출력
    ...
    def find_best_match(self, embedding):
        ...
        if best_similarity > 0.5:  # 유사도가 0.5보다 크면
...
class RecordingThread(QThread):
    ...
    def __init__(self, folder_path):
        ...
        self.recordings = []  # 녹음된 오디오 데이터를 저장할 리스트
    ...
    def run(self):
        ...
        with mic as source:
            ...
            while self.is_recording and not self.stop_event.is_set():
                ...
                self.recordings.append(audio.get_wav_data())
    ...
    def save_last_15_seconds(self):
        if self.recordings:
            ...
            for recording in self.recordings[-15:]:
                audio_segment = AudioSegment(recording)
                combined_audio += audio_segment

            if len(combined_audio) > 15000:
                combined_audio = combined_audio[-15000:]

            combined_audio.export(os.path.join(self.folder_path, "conversation.mp3"), format="mp3")
...
class OpenAi:
    ...
    # 음성을 텍스트로 변환하는 함수
    def voice_to_text(self, only_stop=False):
    ...
    def start_recording(self, folder_path):
        self.recording_thread = RecordingThread(folder_path)
    ...
    def save_last_15_seconds(self):
        ...
        if os.path.exists(audio_path):
            ...
            if len(audio) > 15000:  # 15초보다 긴 경우
                audio = audio[-15000:]  # 마지막 15초만 저장
    ...
    def play_previous_conversation(self, folder_path):
        ...
        if os.path.exists(conversation_path):
            ...
            audio = AudioSegment.from_mp3(conversation_path)
            play(audio)
        else:
...
class MainWindow(QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        # OpenAi 인스턴스 생성
        self.openai_api_key = 'sk-proj-SpR6LbFSbHU3EBBQhlyET3BlbkFJu4ovvJN2xgopPJMG4CzO'  # 자신의 OpenAI API 키로 변경
                            ...
                            if len(face_distances) > 0:  # 얼굴 거리가 존재하는지 확인
                                ...
                                if matches[best_match_index]:
                                    ...
                                    if 0 <= center_x < depth_image.shape[1] and 0 <= center_y < depth_image.shape[0]:
                                        ...
                                        if distance > 10:  # 1미터에서 10미터 사이가 아닌 경우 무시
                                            continue
                                    else:
                                        continue
            ...
            with self.speech_lock:
                if not self.is_speech_paused:  # 음성 안내가 일시정지되지 않았을 때만 실행
                    ...
                    if use_pyttsx3:
                        ...
                        Thread(target=self.ai.text_to_voice, args=(message, 1.0)).start()
    ...
    def closeEvent(self, event):
        ...
        if self.ai.is_recording:
            self.ai.save_last_15_seconds()
        event.accept()
